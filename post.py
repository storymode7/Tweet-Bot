...
def setUpdate(urlfile):
    try:
        ...
        if pos == filelen:  # Means if pos is at end of file
            choice = input('The specified file was finished in last iteration.'
                           'Want to restart it? [y/n]: ')
            if choice.lower() in ['y', 'yes']:
                pos = 0
            else:
                click.echo('Please specify the new file as the argument by'
                           'running the command again.\n')
                exit()
        elif pos > filelen:
            choice = input('The file has been modified to one with shorter' 
                           'lengh. Star from beginning? [y/n]: ')
            if choice.lower() in ['y', 'yes']:
                pos = 0
            else:
                click.echo('Please specify the new file as the argument by'
        ...
        urlfile.seek(pos)
            ...
            click.echo('The position file is tampered.'
...
def printUpdate(update):
    ...
    seperator_string = (
      seperator_variable*seperator_variable_length +
      message + seperator_variable*(seperator_variable_length+offset)
    )
