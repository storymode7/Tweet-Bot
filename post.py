...
def setUpdate(urlfile):
    try:
        ...
        if pos == filelen:  # Means if pos is at end of file
            choice = input(
                'The specified file was finished in last iteration.'
                'Want to restart it? [y/n]: '
            )
            if choice.lower() in ['y', 'yes']:
                pos = 0
            else:
                click.echo(
                    'Please specify the new file as the argument by'
                    'running the command again.\n'
                )
                exit()
        elif pos > filelen:
            choice = input(
                'The file has been modified to one with shorter lengh.'
                'Star from beginning? [y/n]: '
            )
            if choice.lower() in ['y', 'yes']:
                pos = 0
            else:
        ...
        urlfile.seek(pos)
            ...
            click.echo(
                'The position file is tampered.'
                'Starting from beginning.\n'
            )
    except FileNotFoundError:
            click.echo(
                'First time usage. No existing file positions found\n'
            )
    update = ''
    line = urlfile.readline()
    if not pos:
        """
        Since pos not defined, means first iteration,
        so skip to place with first link
        """
        while not line.startswith(('https://', 'http://')):
            """Get to the place with first link"""
            line = urlfile.readline()

    update += line
    line = urlfile.readline()
    while line and not line.startswith(('https://', 'http://')):
        """Store all upto next link"""
        update += line
        line = urlfile.readline()
...
def printUpdate(update):
    ...
    seperator_variable = '-'
    offset = 79 - (seperator_variable_length*2 + len(message))
    seperator_string = (
        seperator_variable*seperator_variable_length + message +
        seperator_variable*(seperator_variable_length+offset)
    )
